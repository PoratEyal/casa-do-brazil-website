...
import { translations, faqCtaHref, type Language } from '../config';
...
export function FAQ({ currentLang }: FAQProps) {
  ...
  if (!faq.titleRegular && faq.faqs.length === 0) return null;
      ...
      <div className="max-w-4xl mx-auto px-6 md:px-12">
        ...
        <div ref={headerRef} className="text-center mb-16 md:mb-20 opacity-0">
          ...
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-sans font-bold text-white tracking-tight">
            {faq.titleRegular} <span className="font-serif italic font-normal text-white/80">{faq.titleItalic}</span>
          </h2>
        </div>
            ...
            {faq.faqs.map((item) => (
              ...
              >
                <AccordionTrigger className="px-6 py-5 hover:no-underline hover:bg-forest-mid/80 transition-colors duration-300 group">
                  <span className="text-start font-sans font-medium text-white text-base md:text-lg pr-4 group-hover:text-white/90">
                    {item.question}
                  </span>
                </AccordionTrigger>
                <AccordionContent className="px-6 pb-6 pt-0">
                  <p
                    className="text-white/60 font-body text-sm md:text-base leading-relaxed text-start"
